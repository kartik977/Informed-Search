import sys #To extract command line arguments and for exit function
...
class node: #This is the data structure for the node.
	...
	def __cmp__(self,other): #this is used to make the node a comparable object to make the fringe sortable (for uninf uses g and for inf use f)
		if self.uSearch:
...
def main():	
	dumpValues = False #set this to True to dump fringe and closed after every loop (For generating Traces).
	if dumpValues:
	...
	while len(fringe) > 0:
		...
		nPop = nPop + 1
		if dumpValues:
			print 'Popping Node '+str(nPop)+': '+str(n)
